...
ARTIFACT_DIR = PROJECT_ROOT / "data" / "stage7_component_canonicalization"
SUMMARY_PATH = PROJECT_ROOT / "docs" / "results" / "component_canonicalization_batch_summary.md"
RUN_PRESETS = [
    "canonical_component_only_single_stage_epoch7_lr2e4",
    "canonical_action_component_single_stage_epoch5_lr2e4",
    "canonical_action_component_single_stage_epoch7_lr2e4",
    "canonical_action_component_structure_then_semantics_stage2_epoch9",
]
SKIP_COMPLETED = True

PRESETS = {
    "canonical_component_only_single_stage_epoch7_lr2e4": {
        "experiment_name": "qwen25_3b_stage7_canonical_component_only_single_stage_epoch7_lr2e4",
        "mode": "full",
        "canonicalization_mode": "component_only",
        "learning_rate": 2e-4,
        "epochs": 7,
        "batch_size": 8,
        "grad_accum": 4,
    ...
    "canonical_action_component_single_stage_epoch5_lr2e4": {
        "experiment_name": "qwen25_3b_stage7_canonical_action_component_single_stage_epoch5_lr2e4",
        "mode": "full",
        "canonicalization_mode": "action_and_component",
        "learning_rate": 2e-4,
        "epochs": 5,
        "batch_size": 8,
        "grad_accum": 4,
    ...
    "canonical_action_component_single_stage_epoch7_lr2e4": {
        "experiment_name": "qwen25_3b_stage7_canonical_action_component_single_stage_epoch7_lr2e4",
        "mode": "full",
        "canonicalization_mode": "action_and_component",
        "learning_rate": 2e-4,
        "epochs": 7,
    ...
    "canonical_action_component_structure_then_semantics_stage2_epoch9": {
        "experiment_name": "qwen25_3b_stage7_canonical_action_component_structure_then_semantics_stage2_epoch9",
...
def canonicalize_record(record: dict, component_map: dict[str, str], mode: str) -> dict:
...
def component_mapping_stats(records: list[dict]) -> dict[str, float]:
    counts: dict[str, Counter] = defaultdict(Counter)
    for record in records:
        target = record["target_json"]
        counts[target["affected_systems"][0]["name"]][target["affected_systems"][0]["component"]] += 1
    total_names = len(counts)
    deterministic_names = sum(1 for counter in counts.values() if len(counter) == 1)
    weighted_majority = 0
    weighted_total = 0
    for counter in counts.values():
        total = sum(counter.values())
        weighted_total += total
        weighted_majority += counter.most_common(1)[0][1]
    return {
        "unique_names": total_names,
        "deterministic_name_fraction": deterministic_names / total_names if total_names else 0.0,
        "weighted_majority_fraction": weighted_majority / weighted_total if weighted_total else 0.0,
    }
...
def run():
    ...
    print("train =", len(raw_train_records))
    print("val =", len(raw_val_records))
    print("test =", len(raw_test_records))
    print("component_mapping_stats =", component_mapping_stats(raw_train_records))
    ...
    for preset_name in RUN_PRESETS:
        ...
        train_records = [canonicalize_record(record, component_map, config["canonicalization_mode"]) for record in raw_train_records]
        val_records = [canonicalize_record(record, component_map, config["canonicalization_mode"]) for record in raw_val_records]
        test_records = [canonicalize_record(record, component_map, config["canonicalization_mode"]) for record in raw_test_records]
    ...
    lines = [
        "# Component Canonicalization Batch Summary",
        "",
    ...
    for item in batch_run_results:
        ...
        lines.extend(
            [
                ...
                f"- mode: `{item['mode']}`",
                f"- canonicalization_mode: `{item['canonicalization_mode']}`",
