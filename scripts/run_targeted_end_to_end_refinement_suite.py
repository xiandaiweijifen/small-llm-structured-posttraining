...
ARTIFACT_DIR = PROJECT_ROOT / "data" / "stage5_targeted_refinement"
SUMMARY_PATH = PROJECT_ROOT / "docs" / "results" / "targeted_end_to_end_refinement_batch_summary.md"
...
PRESETS = {
    "refine_action_component_half_epoch1_lr5e5": {
        "experiment_name": "qwen25_3b_stage5_refine_action_component_half_epoch1_lr5e5",
    ...
    "refine_action_component_half_epoch075_lr5e5": {
        "experiment_name": "qwen25_3b_stage5_refine_action_component_half_epoch075_lr5e5",
        "subset_name": "action_or_component",
        "subset_ratio": 0.5,
        "learning_rate": 5e-5,
        "epochs": 0.75,
    ...
    "refine_action_component_full_epoch05_lr5e5": {
        "experiment_name": "qwen25_3b_stage5_refine_action_component_full_epoch05_lr5e5",
        "subset_name": "action_or_component",
        "subset_ratio": 1.0,
        "learning_rate": 5e-5,
        "epochs": 0.5,
    ...
    "refine_action_component_half_epoch1_lr3e5": {
        "experiment_name": "qwen25_3b_stage5_refine_action_component_half_epoch1_lr3e5",
        "subset_name": "action_or_component",
        "subset_ratio": 0.5,
        "learning_rate": 3e-5,
        "epochs": 1.0,
    ...
    "refine_action_half_epoch075_lr5e5": {
        "experiment_name": "qwen25_3b_stage5_refine_action_half_epoch075_lr5e5",
...
def sample_target_subset(records: list[dict], ratio: float, seed: int) -> list[dict]:
    if ratio >= 0.999:
        return list(records)
...
def run():
    ...
    for preset_name in RUN_PRESETS:
        ...
        sampled_target_subset = sample_target_subset(
            targeted_subsets[config["subset_name"]],
            float(config["subset_ratio"]),
            int(config["seed"]),
        )
        train_subset = list(train_records) + sampled_target_subset
        random.Random(int(config["seed"])).shuffle(train_subset)
        print("sampled_target_subset_size =", len(sampled_target_subset))
    ...
    for item in batch_run_results:
        ...
        lines.extend(
            [
                ...
                f"- subset ratio: `{item['subset_ratio']}`",
