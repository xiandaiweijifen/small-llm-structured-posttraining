...
ARTIFACT_DIR = PROJECT_ROOT / "data" / "stage11_semantic_core_intermediate"
SUMMARY_PATH = PROJECT_ROOT / "docs" / "results" / "semantic_core_intermediate_batch_summary.md"
RUN_PRESETS = [
    "semantic_core_single_stage_epoch7_lr2e4",
    "semantic_core_single_stage_epoch9_lr2e4",
    "semantic_core_structure_then_semantics_stage2_epoch9",
]
SKIP_COMPLETED = True
...
PRESETS = {
    "semantic_core_single_stage_epoch7_lr2e4": {
        "experiment_name": "qwen25_3b_stage11_semantic_core_single_stage_epoch7_lr2e4",
        "mode": "full",
        "learning_rate": 2e-4,
        "epochs": 7,
        "batch_size": 8,
    ...
    "semantic_core_single_stage_epoch9_lr2e4": {
        "experiment_name": "qwen25_3b_stage11_semantic_core_single_stage_epoch9_lr2e4",
        "mode": "full",
        "learning_rate": 2e-4,
        "epochs": 9,
        "batch_size": 8,
    ...
    "semantic_core_structure_then_semantics_stage2_epoch9": {
        "experiment_name": "qwen25_3b_stage11_semantic_core_structure_then_semantics_stage2_epoch9",
...
def to_semantic_core_target(record: dict, component_map: dict[str, str]) -> dict:
    ...
    return {
        ...
        "affected_system_name": target["affected_systems"][0]["name"],
        "affected_component": target["affected_systems"][0]["component"],
        "action_template": ACTION_TEMPLATE_BY_CATEGORY[target["category"]],
        "constraints": deepcopy(target["constraints"]),
    }


def to_semantic_core_record(record: dict, component_map: dict[str, str]) -> dict:
    updated = deepcopy(record)
    updated["target_json"] = to_semantic_core_target(record, component_map)
    updated["schema_name"] = "ticket_semantic_core_v1"
    return updated


def render_semantic_core_prediction(prediction_json: dict | None) -> dict | None:
    if not isinstance(prediction_json, dict):
        return prediction_json
    summary = prediction_json.get("summary")
    category = prediction_json.get("category")
    priority = prediction_json.get("priority")
    requires_followup = prediction_json.get("requires_followup")
    affected_name = prediction_json.get("affected_system_name")
    affected_component = prediction_json.get("affected_component")
    constraints = prediction_json.get("constraints")
    ...
    if not isinstance(requires_followup, bool):
        return prediction_json
    if not isinstance(affected_name, str):
        return prediction_json
    if not (isinstance(affected_component, str) or affected_component is None):
        return prediction_json
    if not isinstance(constraints, dict):
        return prediction_json

    action_text = f"{ACTION_PREFIX_BY_CATEGORY.get(category, 'Investigate issue')}: {summary}"
    return {
        "summary": summary,
        "category": category,
        "priority": priority,
        "requires_followup": requires_followup,
        "affected_systems": [{"name": affected_name, "component": affected_component}],
        "actions_requested": [{"action": action_text, "owner": None, "deadline": None}],
        "constraints": {
            "environment": constraints.get("environment"),
            "blocking": constraints.get("blocking"),
        },
    }
...
def build_semantic_core_user_prompt(input_text: str, include_schema_definition: bool = False) -> str:
    prompt = (
        "Task: extract a semantic core representation for ticket_structured_output.\n"
        "Schema name: ticket_semantic_core_v1\n"
    )
    if include_schema_definition:
        prompt += (
            "Semantic core fields:\n"
            '{"summary": string, "category": enum["bug","feature","question","incident","task"], '
            '"priority": enum["low","medium","high","urgent"], "requires_followup": boolean, '
            '"affected_system_name": string, "affected_component": string|null, '
            '"action_template": enum["ACTION_HANDLE_REQUEST","ACTION_INVESTIGATE_ISSUE","ACTION_REVIEW_AND_PLAN",'
            '"ACTION_INVESTIGATE_AND_MITIGATE_INCIDENT","ACTION_ANSWER_AND_CLARIFY"], '
            '"constraints": {"environment": "prod|staging|dev|null", "blocking": boolean|null}}\n'
        )
    prompt += (
...
def to_sft_record(sample: dict, include_schema_definition: bool = False) -> dict:
    ...
    return {
        ...
        "messages": [
            ...
            {"role": "user", "content": build_semantic_core_user_prompt(sample["input_text"], include_schema_definition)},
            {"role": "assistant", "content": assistant_content},
        ],
    }


def convert_to_sft_records_local(samples: list[dict], include_schema_definition: bool = False) -> list[dict]:
    return [to_sft_record(sample, include_schema_definition=include_schema_definition) for sample in samples]
...
def write_sft_split(records: list[dict], path: Path, include_schema_definition: bool) -> None:
    sft_records = convert_to_sft_records_local(records, include_schema_definition=include_schema_definition)
...
def build_inference_messages(record: dict) -> list[dict]:
    return [
        {"role": "system", "content": build_system_prompt()},
        {"role": "user", "content": build_semantic_core_user_prompt(record["input_text"], include_schema_definition=False)},
    ]
...
def run():
    ...
    train_records = [to_semantic_core_record(record, component_map) for record in raw_train_records]
    val_records = [to_semantic_core_record(record, component_map) for record in raw_val_records]
    test_records = [to_semantic_core_record(record, component_map) for record in raw_test_records]
                ...
                predictions.append(
                    {
                        ...
                        "prediction_json": render_semantic_core_prediction(prediction_json),
    ...
    lines = [
        "# Semantic Core Intermediate Batch Summary",
        "",
