...
BASELINE_CHECKPOINT_DIR = PROJECT_ROOT / "results" / "checkpoints" / "qwen25_3b_stage14_target_allcore_x1_epoch1_lr5e5"
...
ARTIFACT_DIR = PROJECT_ROOT / "data" / "stage15_external_overnight_optimization"
SUMMARY_PATH = PROJECT_ROOT / "docs" / "results" / "external_overnight_optimization_batch_summary.md"
TRAINING_PRESETS = [
...
INFERENCE_BATCH_SIZE = 28

PRESETS = {
    "target_allcore_x2_epoch1_lr5e5": {
        "experiment_name": "qwen25_3b_stage15_target_allcore_x2_epoch1_lr5e5",
    ...
    "target_allcore_x1_epoch2_lr5e5": {
        "experiment_name": "qwen25_3b_stage15_target_allcore_x1_epoch2_lr5e5",
        "subset_name": "all_core_fields",
        "learning_rate": 5e-5,
        "epochs": 2,
        "target_repeat": 1,
    ...
    "target_allcore_x1_epoch1_lr3e5": {
        "experiment_name": "qwen25_3b_stage15_target_allcore_x1_epoch1_lr3e5",
        "subset_name": "all_core_fields",
        "learning_rate": 3e-5,
        "epochs": 1,
        "target_repeat": 1,
    ...
    "target_allcore_x2_epoch1_lr3e5": {
        "experiment_name": "qwen25_3b_stage15_target_allcore_x2_epoch1_lr3e5",
        "subset_name": "all_core_fields",
        "learning_rate": 3e-5,
        "epochs": 1,
        "target_repeat": 2,
    ...
    "target_component_category_x2_epoch2_lr5e5": {
        "experiment_name": "qwen25_3b_stage15_target_component_category_x2_epoch2_lr5e5",
    ...
    "target_action_category_x1_epoch1_lr5e5": {
        "experiment_name": "qwen25_3b_stage15_target_action_category_x1_epoch1_lr5e5",
        "subset_name": "action_or_category",
        "learning_rate": 5e-5,
        "epochs": 1,
        "target_repeat": 1,
        "batch_size": 14,
...
def build_targeted_subsets(train_records: list[dict], mined_rows: list[dict]) -> dict[str, list[dict]]:
    ...
    for row in mined_rows:
        ...
        if fields.intersection({"actions_requested[0].action", "category"}):
            subsets["action_or_category"].append(record)
...
def apply_postprocess_variant(prediction_json: dict | None, preset_name: str, strict_map: dict[str, str], majority_map: dict[str, str]) -> dict | None:
...
def run():
    ...
    strict_map, majority_map = build_component_maps(external_train)
    ...
    for preset_name in POSTPROCESS_PRESETS:
        experiment_name = f"qwen25_3b_stage15_extpp_{preset_name}"
        paths = build_output_paths(experiment_name)
        print("\n" + "=" * 80)
        ...
        for record in source_predictions:
            prediction_json = apply_postprocess_variant(record.get("prediction_json"), preset_name, strict_map, majority_map)
            predictions.append(
                {
                    **record,
                    "prediction_json": prediction_json,
    ...
    postprocess_leaderboard = sorted(
        [
            {
                "preset_name": item["preset_name"],
                "experiment_name": item["experiment_name"],
    ...
    print("training_leaderboard =")
    for row in training_leaderboard:
        print(row)
    print("postprocess_leaderboard =")
    for row in postprocess_leaderboard:
        print(row)
